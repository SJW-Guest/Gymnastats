...
interface Gymnast { id: string; first_name: string; last_name: string; age_group: string; current_team_id: string }
...
export default function LineupPage() {
  ...
  useEffect(() => {
    async function init() {
      ...
      setMeets(m ?? [])
      setTeams(t ?? [])
      if (m && m.length > 0) setSelectedMeet(m[0].id)
      if (t && t.length > 0) setSelectedTeam(t[0].id)
      setLoading(false)
    }
    init()
  }, [])
  ...
  const loadLineup = useCallback(async (meetId: string, teamId: string) => {
    ...
    const entries: LineupEntry[] = (gymnasts ?? []).map(g => ({
      ...
      status: (lineupMap[g.id]?.status ?? 'active') as 'active' | 'scratched',
  ...
  useEffect(() => { if (selectedMeet && selectedTeam && teams.length > 0) loadLineup(selectedMeet, selectedTeam) }, [selectedMeet, selectedTeam, teams, loadLineup])

  function moveUp(idx: number) {
    if (idx === 0) return
    const newLineup = [...lineup]
    ;[newLineup[idx - 1], newLineup[idx]] = [newLineup[idx], newLineup[idx - 1]]
    setLineup(newLineup.map((e, i) => ({ ...e, running_order: i + 1 })))
  }
  ...
  function toggleInLineup(gymnast_id: string) {
    setLineup(prev => {
      ...
      const gymnast = roster.find(g => g.id === gymnast_id)!
      const team = teams.find(t => t.id === selectedTeam)
      return [...prev, { gymnast_id, first_name: gymnast.first_name, last_name: gymnast.last_name, age_group: gymnast.age_group, team_name: team?.name ?? '', running_order: prev.length + 1, status: 'active', is_in_lineup: true }]
    })
  }
    ...
    <div style={s.page}>
      ...
      <main style={s.main}>
        <div style={s.header}>
          ...
          <button onClick={saveLineup} disabled={saving || isLocked} style={{...s.primaryBtn, ...(saved ? s.savedBtn : {})}}>
        ...
        <div style={s.card}>
          ...
          {lineup.length === 0 ? (
            ...
            <div key={entry.gymnast_id} style={{...s.row, ...(entry.status === 'scratched' ? s.scratchedRow : {})}}>
              ...
              <div style={s.colName}>
                <span style={{...s.gymnName, ...(entry.status === 'scratched' ? {textDecoration:'line-through', opacity:0.5} : {})}}>{entry.last_name}, {entry.first_name}</span>
              </div>
              ...
              <div style={s.colStatus}>
                ...
                {entry.status === 'active' && isLocked && <span style={{fontSize:'12px',color:'#10b981'}}>Competing</span>}
              </div>
              {!isLocked && (
                <div style={s.colActions}>
                  ...
                  <button onClick={() => toggleInLineup(entry.gymnast_id)} style={s.removeBtn} title="Remove from lineup">×</button>
                </div>
              )}
            </div>
          ))}

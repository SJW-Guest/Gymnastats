...
export default function LineupManagerPage() {
    ...
    async function load() {
      ...
      if (user) {
        const { data: userData } = await supabase
          .from('users').select('club_id, role, full_name').eq('id', user.id).single()
  ...
  // Host sees all lineup entries; guest sees only their club's teams
  const visibleLineup = isHost
    ? lineup
    : lineup.filter(e => e.team_club_id === userClubId)

  // Teams derived from visible lineup only
  const teams = Array.from(new Map(visibleLineup.map(e => [e.team_id, e.team_name])).entries())
    .map(([id, name]) => ({ id, name }))

  const filtered = filterTeamId === 'all'
    ? visibleLineup
    : visibleLineup.filter(e => e.team_id === filterTeamId)
  ...
  // Host can scratch anyone; guest can only scratch their own teams
  function canEdit(entry: LineupEntry): boolean {
    if (isHost) return true
    return entry.team_club_id === userClubId
  }
  ...
  const scratchedCount = visibleLineup.filter(e => e.status === 'scratched').length
        ...
        <main style={s.main}>
          <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',flexWrap:'wrap',gap:12,marginBottom:4}}>
            <div>
              ...
              <p style={s.pageSub}>
                {visibleLineup.length} gymnasts{scratchedCount > 0 ? ` · ${scratchedCount} scratched` : ''}
                {!isHost && <span style={{marginLeft:8,fontSize:12,color:'#9ca3af'}}>(your teams only)</span>}
              </p>
            </div>
          ...
          {visibleLineup.length === 0 ? (
            <div style={s.emptyState}>
              ...
              <p style={{fontSize:13,color:'#9ca3af',margin:0}}>
                {isHost
                  ? 'Teams need to submit their lineups before gymnasts appear here.'
                  : 'Your teams have not submitted a lineup for this meet yet.'}
              </p>
            </div>
          ) : (
            groups.map(group => (
              <div key={group.label} style={s.card}>
                ...
                <div style={{...s.tableHeader, gridTemplateColumns: `40px 2fr 1fr${groupBy === 'age_group' ? ' 1fr' : ''} 1fr 80px`}}>
                  <span>#</span>
                  <span>Gymnast</span>
                  <span>Age group</span>
                ...
                {group.entries.map(entry => {
                  ...
                  return (
                    <div key={entry.id} style={{...s.tableRow, gridTemplateColumns: `40px 2fr 1fr${groupBy === 'age_group' ? ' 1fr' : ''} 1fr 80px`, ...(scratched ? s.scratchedRow : {})}}>
...
const s: Record<string, React.CSSProperties> = {
  ...
  userName:     {color:'#9ca3af',fontSize:13,marginRight:8},
  ...
  tableHeader:  {display:'grid',gridTemplateColumns:'40px 2fr 1fr 1fr 80px',gap:12,padding:'6px 8px',fontSize:11,fontWeight:600,color:'#9ca3af',textTransform:'uppercase',letterSpacing:'0.05em',marginBottom:4},
  tableRow:     {display:'grid',gridTemplateColumns:'40px 2fr 1fr 1fr 80px',gap:12,padding:'10px 8px',borderRadius:8,alignItems:'center',borderBottom:'1px solid #f3f4f6'},

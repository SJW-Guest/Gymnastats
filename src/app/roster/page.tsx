...
export default function RosterPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // get logged-in user's club_id, role, name
      ...
      if (!userData?.club_id) { setLoading(false); return }

      // fetch only this club's teams
      const { data: t } = await supabase
        .from('teams')
        .select('id, name, division_group')
      ...
      if (clubTeams.length > 0) {
        setForm(f => ({ ...f, current_team_id: clubTeams[0].id }))
        // fetch only gymnasts on this club's teams
        const teamIds = clubTeams.map(t => t.id)
        const { data: g } = await supabase
          .from('gymnasts')
          .select('id, first_name, last_name, age_group, current_team_id, teams(name)')
  ...
  async function deleteGymnast(id: string, name: string) {
    if (!confirm(`Remove ${name} from the roster? This cannot be undone.`)) return
    setDeleting(id)
    const { createBrowserClient } = await import('@supabase/ssr')
    const supabase = createBrowserClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!)
    await supabase.from('gymnasts').update({ is_active: false }).eq('id', id)
  ...
  const ageColors: Record<string, string> = {
    Novice: '#f59e0b', Children: '#10b981', Junior: '#6366f1', Senior: '#ef4444'
  }
      ...
      <main style={s.main}>
        <div style={s.header}>
          <div>
            ...
            <p style={s.sub}>{filtered.length} gymnast{filtered.length !== 1 ? 's' : ''} · {teams.length} team{teams.length !== 1 ? 's' : ''}</p>
          </div>
          ...
          {filtered.length === 0 ? (
            ...
            <div key={g.id} style={s.tableRow}>
              ...
              <span style={{textAlign:'right'}}>
                <button
                  onClick={() => deleteGymnast(g.id, `${g.first_name} ${g.last_name}`)}
...
const s: Record<string, React.CSSProperties> = {
  ...
  deleteBtn:  { background: 'none', border: '1px solid #fecaca', color: '#ef4444', borderRadius: '6px', padding: '4px 10px', fontSize: '12px', cursor: 'pointer', fontWeight: 500 },
  ...
  tableHead:  { display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 80px', gap: '1rem', padding: '10px 16px', background: '#f8f9fb', fontSize: '11px', fontWeight: 600, color: '#64748b', textTransform: 'uppercase', letterSpacing: '0.05em', borderBottom: '0.5px solid #e5e7eb' },
  tableRow:   { display: 'grid', gridTemplateColumns: '2fr 2fr 1fr 80px', gap: '1rem', padding: '12px 16px', borderBottom: '0.5px solid #f1f5f9', alignItems: 'center' },
